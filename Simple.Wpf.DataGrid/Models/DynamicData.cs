...
public sealed class DynamicData : DynamicObject, ICloneable<DynamicData>, IEnumerable
{
    ...
    public DynamicData(int size) => _dictionary = new Dictionary<string, object>(size);
    ...
    public DynamicData Clone() => new DynamicData(_dictionary.ToDictionary(x => x.Key, x => x.Value));
    ...
    public override bool TrySetMember(SetMemberBinder binder, object value) => Update(binder.Name.ToLower(), value);
    ...
    public void Add(string name, object value) => Update(name, value);
}