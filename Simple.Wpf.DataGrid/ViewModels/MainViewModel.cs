...
public sealed class MainViewModel : BaseViewModel, IMainViewModel
{
        ...
        ISchedulerService schedulerService)
    {
        ...
        ColumnPickerCommand.Subscribe(_ => ShowColumnPicker())
            .DisposeWith(this);

        _suspendNotifications = new SerialDisposable()
            .DisposeWith(this);

        FilterChanged.Subscribe(_ => _collectionView.Refresh())
            .DisposeWith(this);

        ColumnsChanged.ObserveOn(schedulerService.Dispatcher)
            .Subscribe(_ => OnPropertyChanged(nameof(VisibleColumns)))
            .DisposeWith(this);
